 ...
 * Parameters: store_2, Truck objects
 * Return: void
 */
...
// loop through all the results and connect to the first we can
	...
	if (p->ai_family == PF_INET) {
		...
		inet_ntop(p->ai_family, get_in_addr((struct sockaddr *) &my_addr), s,
				sizeof s);
		printf("Phase 1: store_2 has TCP port number %d and IP address %s \n",
				my_addr.sin_port, s);
	...
	store newStore;
	getStoreData("Store-2.txt", &newStore);
	...
	printf(
			"Phase 1: The outlet vector <%d,%d,%d> for Store_2 has been sent to the central warehouse\n",
			newStore.Cameras, newStore.Laptops, newStore.Printers);
	printf("End of Phase 1 for Store_2\n");

	/*********** UDP-1: Get the Truck from the Store_1 *************/
	memset(&hints, 0, sizeof hints);
	...
	inet_ntop(p->ai_family, get_in_addr((struct sockaddr *) p->ai_addr), s,
			sizeof s);
	printf("\nPhase 2: Store_2 has UDP port %d and IP address %s \n",
			atoi(MY_UDP1PORT), s);
	...
	printf(
			"Phase 2: Store_2 received the truck-vector <%d,%d,%d> from Store_1.\n",
	...
	printf(
			"Phase 2: The updated truck-vector<%d,%d,%d> has been sent to Store_3\n",
			truck.Cameras, truck.Laptops, truck.Printers);
