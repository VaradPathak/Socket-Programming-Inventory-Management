 ...
 * Parameters: store_4, Truck objects
 * Return: void
 */
...
// loop through all the results and connect to the first we can
	...
	store newStore;
	getStoreData("Store-4.txt", &newStore);
	...
	printf(
			"Phase 1: The outlet vector <%d,%d,%d> for Store_4 has been sent to the central warehouse\n",
			newStore.Cameras, newStore.Laptops, newStore.Printers);
	printf("End of Phase 1 for Store_4\n");

	/*********** UDP-1: Get the Truck from the Store_3 *************/
	memset(&hints, 0, sizeof hints);
	...
	inet_ntop(p->ai_family, get_in_addr((struct sockaddr *) p->ai_addr), s,
			sizeof s);
	printf("\nPhase 2: Store_4 has UDP port %d and IP address %s \n",
			atoi(MY_UDP1PORT), s);
	...
	printf(
			"Phase 2: Store_4 received the truck-vector <%d,%d,%d> from Store_3.\n",
	...
	/********* UDP-2: Forward the Truck to store_1 *****************/
	memset(&hints, 0, sizeof hints);
	hints.ai_family = AF_UNSPEC;
	hints.ai_socktype = SOCK_DGRAM;
	...
	printf(
			"Phase 2: The updated truck-vector<%d,%d,%d> has been sent to Store_1\n",
			truck.Cameras, truck.Laptops, truck.Printers);
	...
	/********** UDP-3: Get the Truck from Store_3 ***********/
	memset(&hints, 0, sizeof hints);
	hints.ai_family = AF_UNSPEC; // set to AF_INET to force IPv4
	hints.ai_socktype = SOCK_DGRAM;
	...
	/*********** UDP-4: Forward the Truck to warehouse *****************/
	memset(&hints, 0, sizeof hints);
	hints.ai_family = AF_UNSPEC;
	hints.ai_socktype = SOCK_DGRAM;
	...
	freeaddrinfo(servinfo);
	printf(
			"Phase 2: The updated truck-vector<%d,%d,%d> has been sent to the central warehouse\n",
			truck.Cameras, truck.Laptops, truck.Printers);
	printf("Phase 2: Store_4 updated outlet vector is <%d,%d,%d>\n\n",
			newStore.Cameras, newStore.Laptops, newStore.Printers);
